...
- hint 종류:
  ...
  - "number"
  - "default"
- 형 변환 시, `[Symbol.toPrimitive](hint)` 메서드가 있다면 호출한다.
  ```javascript
  let user = {
    name: "John",
    money: 1000,
  ...
  ```
- 없고, hint가 string이라면 `obj.toString()`이나 `obj.valueOf()`를 호출한다.
- 없고, hint가 number나 default라면 `obj.valueOf()`나 `obj.toString()`을 호출한다.
  ```javascript
  let user = {
    name: "John",
    money: 1000,
